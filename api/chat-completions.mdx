...
description: "Send a text generation request to Kyara Intelligence. Supports streaming via Server-Sent Events and all standard OpenAI sampling parameters."
---

The chat completions endpoint accepts standard OpenAI-format requests and returns text responses. You can connect any OpenAI-compatible client by pointing it at the Kyara base URL. The endpoint is text-only — see [Unsupported Features](/reference/unsupported-features) for a full list of capabilities such as tool calling that are not available.
...
<ParamField body="reasoning" type="object">
  ...
  - `effort` (string) — e.g. `"low"`, `"medium"`, or `"high"`
  - `max_tokens` (integer) — maximum tokens to spend on reasoning

  Use `reasoning.effort` rather than the top-level `reasoning_effort`, which is silently ignored.
</ParamField>

<ParamField body="n" type="integer">
  Number of completion choices to generate for the request. Defaults to `1`.
</ParamField>
