...
<Update label="0.4" description="August 1, 2026" tags={["API responses"]}>
  A small response cleanup release.

  - **`system_fingerprint` is no longer returned.** The `system_fingerprint` field is now stripped from chat completion responses, both non-streaming responses and every streaming chunk. It exposed upstream infrastructure details and carried no meaning for Kyara requests.
</Update>
