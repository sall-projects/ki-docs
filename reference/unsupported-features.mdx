---
title: "Kyara Unsupported Features, Endpoints, and Parameters"
sidebarTitle: "Unsupported Features"
description: "Kyara Intelligence is text-in, text-out. This page lists every OpenAI feature, endpoint, and parameter not available on the platform."
---

Kyara supports the chat completions endpoint for text generation. Any OpenAI-compatible client can connect, but the following capabilities are not available on the platform.

## Summary

- **Function / tool calling** — `tools` and `tool_choice` are silently ignored; the model never sees them.
...
<Info>
  The following parameters do not produce an error. For client compatibility, Kyara removes unrecognized top-level parameters from the request rather than rejecting it. Sending `tools` or `response_format` returns a normal text response — the model never sees those fields, so tool calls and structured output simply will not happen.
</Info>
...
- Support for sampling parameters varies per model — check the [catalog](https://kyara-intelligence.com/models).
